...
#                         print ('neighbor is: ' + nn)
                        if nn not in visited:
                            ...
                            try:
                                ...
                                node2parents[nn] = []
                                node2parents[nn].append(n)
                        elif (node2distances[nn] == length ):
                            if(n not in node2parents[nn]):
                                node2num_paths[nn] = node2num_paths[nn] + 1
                                node2parents[nn].append(n)
    node2num_paths[root] = 1  
#     node2distances = dict(sorted(node2distances.items()))
#     node2num_paths = dict(sorted(node2num_paths.items()))
#     node2parents = dict(sorted(node2parents.items()))
#     print (dict(sorted(node2distances.items())))
#     print (dict(sorted(node2num_paths.items())))
#     print (dict(sorted(node2parents.items())))
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for node, parent in node2parents.items():
        number_of_node.add(node)
        #number_of_node.add(root)
        for p in parent:
            nodes[p].append(node)
#      print(nodes)
#      print('___________')
...
# #     calculation of node score
    nodes.pop(root)
    for n in number_of_node:
        node_score[n] = 1
#         print(node_score)
    for node in nodes:
        for a in nodes[node]:
            node_score[node] =  node_score[node] + node_score[a]/node2num_paths[a]
#         print(node_score)
    for node,parents in node2parents.items():
        for p in parents:
            result[tuple(sorted([p, node]))]= node_score[node]/node2num_paths[node]
...
#         print(n)
        ...
        score = bottom_up(n, node2distances, node2num_paths, node2parents)
...
def partition_girvan_newman(graph, max_depth):
    ...
    pass
    copy=graph.copy()
    betweenness = sorted(approximate_betweenness(copy, max_depth).items(), key=lambda x: x[1], reverse=True)
#     print (betweenness)
    components = (get_components(copy))
    
    while len(components) == 1:
        edge_to_remove = betweenness.pop(0)[0]
#         print (edge_to_remove)
        copy.remove_edge(*edge_to_remove)
        components = (get_components(copy))
...
#     print(sorted(components, key= lambda x:len(x), reverse = True))
...
def brute_force_norm_cut(graph, max_size):
    ...
    for i in range(1, max_size+1):
        for combination in combinations(graph.edges(),i):
            copy.remove_edges_from(combination)
            component=get_components(copy)
            if len(component) == 2:
                norm = norm_cut(component[0], component[1], graph)
                brute.append((norm, list(combination)))
            copy.add_edges_from(combination)
    return brute    
...
def score_max_depths(graph, max_depths):
    ...
    depth = []
    for m in max_depths:
        component = partition_girvan_newman(graph, m)
        norm = norm_cut(component[0], component[1], graph)
        depth.append((m, norm))
...
def make_training_graph(graph, test_node, n):
    ...
    ###TODO
    graph1 = graph.copy()
    remove1 = sorted(graph1.edges(test_node))[:n]
    graph1.remove_edges_from(remove1)
    return graph1    
...
def jaccard(graph, node, k):
    ...
    result = sorted(scores, key=lambda x: x[1], reverse=True)[:k]
    return result
...
def evaluate(predicted_edges, graph):
    ...
    for edge1 in predicted_edges:
        if graph.has_edge(*edge1):
            length = length + 1
        score1= length/len(predicted_edges)
...
#         print (node)
        for a in nodes[node]:
            node_score[node] =  node_score[node] + node_score[a]/node2num_paths[a]
#             print ('parent :' + node + str(node_score)) #print(node_score)
    for node,parents in node2parents.items():
        for p in parents:
            result[tuple(sorted([p, node]))]= node_score[node]/node2num_paths[node]
...
if __name__ == '__main__':
    main()
>>>>>>> cd1af8cd6eaaac8c024b2c8f9d98eb026d2ee69e
